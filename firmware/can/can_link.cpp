...
static const int AfrData1Multiplexor = 50;
static const int AfrData2Multiplexor = 51;

// ID: 0x3B6..0x3BD for AFR channels
struct AfrData1
{
    uint8_t FrameIndex = AfrData1Multiplexor; // Fixed value for AFR frames
...
struct AfrData2
{
    uint8_t FrameIndex = AfrData2Multiplexor; // Fixed value for AFR frames
    uint8_t Reserved;
...
void SendLinkAfrFormat(Configuration* configuration, uint8_t ch)
{
    ...
    frame->FrameIndex = linkecu::AfrData1Multiplexor;
    ...
    frame2->FrameIndex = linkecu::AfrData2Multiplexor;
...
void SendLinkEgtFormat(Configuration* configuration)
{
    ...
    // Not sure how offsets work for TCCXX, for now just send message 1
...
void ProcessLinkCanMessage(const CANRxFrame* frame, Configuration* configuration, struct CanStatusData* statusData)
{
    ...
    if (id == LINKECU_L2C_IN_BASE_ID && frame->DLC == 8 && (frame->data8[0]) == 85)
    {
        ...
        (void)exhaustPressure;
        (void)useExhaustPressure;
    }
